...
| EMS focused site workspace | `site/taoyuan-logistics/realtime`, desktop + 375px in-app browser | Exactly three site destinations: live data/metrics/alerts, reports, and contact; one active panel at a time; no horizontal overflow | Three accessible tabs (`即時資料`, `報告`, `站點聯絡資訊`); realtime panel combines PostgreSQL-backed metrics with active alerts; Reports and Contact switch independently; mobile `375px` layout has no overflow | Verified in-app on 2026-07-24 |
